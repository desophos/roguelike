...
class Object:
	...
	def move(self, dx, dy):
		...
		x_in_range = True
		y_in_range = True
		if self.x + dx < 0 or self.x + dx >= MAP_WIDTH: x_in_range = False
		if self.y + dy < 0 or self.y + dy >= MAP_HEIGHT: y_in_range = False

		if x_in_range and y_in_range:
			move_blocked = is_blocked(self.x + dx, self.y + dy)

		# If I am held by a spell or something, I can't move.
		if self.held: move_blocked = True
		...
		if not move_blocked:
			if dx != 0 and dy != 0:
				if x_in_range and y_in_range:
					self.x += dx
					self.y += dy
			elif dx != 0 and x_in_range: self.x += dx
			elif dy != 0 and y_in_range: self.y += dy
...
class Character(Object):
	def __init__(self, x, y, char, name, color, level=1, xp=0, xp_curve=DEFAULT_XP_CURVE, blocks=True, blocks_sight=False, speed=DEFAULT_SPEED, inventory=None, skills=None,
		...
		if self.combatant: self.combatant.owner = self
		self.caster = caster
		if self.caster:
...
class Player(Character):
	...
	def handle_keys(self):
		...
		def switch_key(keys):
			for key in keys:
				if key_event_structure.vk == key: return True
			...
			if switch_key(movement_keys): #player tries to move
				...
				if key_char in inventory_key:
					...
					if chosen_item is not None:
						if chosen_item.use(): return True
				...
				if key_char in char_screen_key:
					...
					libtcod.sys_wait_for_event(libtcod.KEY_PRESSED, key_event_structure, mouse_event_structure, True) # wait for keypress
...
class Caster:
	...
	def cast_spell(self, spell):
		if (self.mp - spell.mp_cost) < 0: return False # if I don't have enough MP, I can't cast the spell
		self.calc_osmosis() # calculate the factors from surrounding terrain
		#print self.hot_cold, self.wet_dry
		success = spell.use()
		self.reset_factors()
		if success: self.change_mp(self.owner, -spell.mp_cost)
		return success
