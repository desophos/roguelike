...
noise_map = [[libtcod.noise_get_fbm(noise2d_gen, [i / float(MAP_WIDTH), j / float(MAP_HEIGHT)], 32.0)
			for i in range(MAP_WIDTH)] for j in range(MAP_HEIGHT)] # map of actual fbm noise values
...
class Tile:
	...
	def __init__(self, tile_type):
		...
		if self.block_sight is None: self.block_sight = self.blocked
...
def generate_river():
	...
	while not libtcod.path_is_empty(path):
		...
		if x_in_min:
			...
			#bottom left
			if y_in_min: g.level_map[y - 1][x - 1] = Tile(tile_types.SHALLOW_WATER)
			#top left
			if y_in_max: g.level_map[y + 1][x - 1] = Tile(tile_types.SHALLOW_WATER)
		#right
		if x_in_max:
			g.level_map[y][x + 1] = Tile(tile_types.SHALLOW_WATER)
			#bottom right
			...
			if y_in_max: g.level_map[y + 1][x + 1] = Tile(tile_types.SHALLOW_WATER)
		#bottom
		if y_in_min: g.level_map[y - 1][x] = Tile(tile_types.SHALLOW_WATER)
		#top
		if y_in_max: g.level_map[y + 1][x] = Tile(tile_types.SHALLOW_WATER)
...
def generate_dungeon():
	...
	g.level_map = [[ Tile(tile_types.CAVE_WALL)
		for x in range(MAP_WIDTH) ]
			for y in range(MAP_HEIGHT) ]
...
def bsp_leaf_create_room(node, data):
	...
	if not libtcod.bsp_is_leaf(node): return False
	...
	except NameError: rooms = [new_room]
