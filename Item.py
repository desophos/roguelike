...
class Inventory(UsableManager):
	...
	def choice_menu(self, key_char):
		...
		if key_char == 'd': return UsableManager.choice_menu(self, "Press a letter to drop an item:", "Inventory is empty.", INVENTORY_WIDTH, ["quantity"])
		elif key_char == 'I': return UsableManager.choice_menu(self, "Press a letter to use an item:", "Inventory is empty.", INVENTORY_WIDTH, ["quantity"])
	
	def add(self, item):
		if item.name not in self.elements.keys(): UsableManager.add(self, item)
...
class Item(Object.Object, Usable):
	...
	def pick_up(self, character):
		...
		if character == Object.player and len(character.inventory.elements) >= 26:
			...
			g.items.remove(self)
			if character == Object.player: display.message('You picked up a ' + self.name + '!', libtcod.green)
	...
	def drop(self):
		...
		self.x = self.owner.x
		self.y = self.owner.y
		if self.owner == Object.player: display.message('You dropped a ' + self.name + '.', libtcod.yellow)
		self.owner = None
