...
class Usable:
	...
	def use(self):
		...
		if not target: display.message("Usage cancelled.")
		else:
			if not self.use_msg and self.verb and self.noun:
				self.use_msg = self.verb + " " + self.noun + "."
			if self.use_msg: display.message(self.owner.name + " " + self.use_msg)
...
class UsableManager:
	...
	def choice_menu(self, header, err_msg, width, params=[]):
		...
		err_msg += "\n"
		ordered_params = []
		if not self.elements: options = [err_msg]
		else:
		...
		if index >= 0 and index < len(options):
			if self.elements: return self.elements.values()[index]
