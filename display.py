from config import \
    SCREEN_HEIGHT, SCREEN_WIDTH, CHAR_INFO_WIDTH, \
    MAP_WIDTH, MAP_HEIGHT, LIMIT_FPS
import globals as g
...
def render_bar(x, y, total_width, name, value, maximum, bar_color, back_color):
    ...
    libtcod.console_rect(
        panel, x, y, total_width, 1, False, libtcod.BKGND_SET
    )
    ...
    if bar_width > 0:
        libtcod.console_rect(
            panel, x, y, bar_width, 1, False, libtcod.BKGND_SET
        )
...
def get_names_under_mouse():
    ...
    #libtcod.console_flush()
    libtcod.sys_check_for_event(libtcod.EVENT_MOUSE_MOVE,
                                g.key_event_structure,
                                g.mouse_event_structure)
    (x, y) = (g.mouse_event_structure.cx, g.mouse_event_structure.cy)

    # create a list with the names of all objects
    # at the mouse's coordinates and in FOV
    objects = []
...
def render_all():
    ...
    if g.fov_recompute:
        ...
        libtcod.map_compute_fov(fov_map, Object.player.x, Object.player.y,
                                TORCH_RADIUS, FOV_LIGHT_WALLS, FOV_ALGO)

        # go through all tiles
        # and set their background color according to the FOV
                ...
                if not visible:
                    ...
                    if g.level_map[y][x].explored:
                        libtcod.console_set_char_background(
                            con, x, y, g.level_map[y][x].color_out_FOV,
                            libtcod.BKGND_SET
                        )
                else:
                    # it's visible
                    libtcod.console_set_char_background(
                        con, x, y, g.level_map[y][x].color_in_FOV,
                        libtcod.BKGND_SET
                    )
    ...
    render_bar(
        1, 1, BAR_WIDTH, 'HP', Object.player.combatant.hp,
        Object.player.combatant.max_hp, libtcod.light_red, libtcod.darker_red
    )
    render_bar(
        1, 2, BAR_WIDTH, 'MP', Object.player.caster.mp,
        Object.player.caster.max_mp, libtcod.light_blue, libtcod.darker_blue
    )
...
libtcod.console_init_root(SCREEN_WIDTH, SCREEN_HEIGHT,
                          'python/libtcod roguelike', False)
